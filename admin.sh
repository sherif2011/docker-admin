...
if [ "$#" -eq 0 ] || [ $1 = "-h" ] || [ $1 = "--help" ]; then
    echo "Usage: ./admin [OPTIONS] COMMAND [arg...]"
    echo "       ./admin [ -h | --help ]"
    echo ""
    echo "Options:"
...
stop_existing() {
  ADMIN="$(docker ps --all --quiet --filter=name=admin)"
  REDIS="$(docker ps --all --quiet --filter=name=admin-redis)"
  POSTGRES="$(docker ps --all --quiet --filter=name=admin-postgres)"
