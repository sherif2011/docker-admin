...
let MySequence = class MySequence {
    constructor(findRoute, parseParams, invoke, send, reject, authenticateRequest, authenticateRequestClient, checkAuthorisation) {
    ...
    async handle(context) {
        try {
            ...
            const isAccessAllowed = await this.checkAuthorisation(authUser && authUser.permissions, request);
            if (!isAccessAllowed) {
...
MySequence = __decorate([
    ...
    __param(6, context_1.inject(loopback4_authentication_1.AuthenticationBindings.CLIENT_AUTH_ACTION)),
    __param(7, context_1.inject(loopback4_authorization_1.AuthorizationBindings.AUTHORIZE_ACTION)),
